/**
 * classes "draggable"
 * must be declared "position: fixed|absolute;"
 */
document.addEventListener("DOMContentLoaded", function (event) {
    function dragElement(sElementId) {
        function closeDragElement() {
            document.onmouseup = null;
            document.onmousemove = null;
        }
        function elementDrag(oEvent) {
        ...
        restore(oElement);
        oElement.onmousedown = dragMouseDown;
    }
    // init all draggable
    for (var oElement of document.getElementsByClassName('draggable')) {
        dragElement(oElement.id);
    }
});