...
/**
 * locates source/binary for a specified command
...
function whereis(string $sWhereIsItem = '')
{
    ...
    {
        ob_start();
        system('/bin/bash -c "type -p which"', $iCode);
        $mWhich = ob_get_contents();
        $sWhich = trim(((false === $mWhich) ? '' : $mWhich));
        ob_end_clean();

        if (false === empty($sWhich))
        {
            $sCmd = $sWhich . ' ' . $sWhereIsItem;
